...
public class GroupCharObjsController_old
{
    ...
    public void SwitchFormation(E_FORMATION_TYPE formation, Vector3 start, Vector3 target)
    {
        bool retCode = false;
        ...
        for (int i = 0; i < m_charObjs.Count; i++)
        {
            ...
            CharObj charObj = m_charObjs[i];
            m_charObjs[i].AI_Arrive(m_charObjs[i].GameObject.transform.position,
                target - (m_formationPoints[i] - start), 2f,
                delegate()
                {
                    Debug.Log("Group执行OnArrived");
                    OnArrived(charObj, target);
